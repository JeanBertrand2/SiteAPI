...
const PaiementFichier = ({
  ...
  {/* RECUPERATION IdentifiantAPI
  const [identifiantAPI, setIdentifiantAPI] = useState("");
        
        const fetchIdentifiantAPI = async () => {
          try {
            const res = await fetch("http://localhost:2083/prestataires/identifiant");
            if (!res.ok) throw new Error("Erreur réseau");
            const data = await res.json();
            return data.identifiantAPI; 
          } catch (err) {
            console.error("Erreur récupération IdentifiantAPI :", err);
            return "";
          }
        };

    useEffect(() => {
        fetchIdentifiantAPI().then(setIdentifiantAPI);
      }, []);
*/}
  ...
  const lireFichierDolibarr = (file) => {
    ...
    reader.onload = (e) => {
      ...
      const formulairesImportés = Object.entries(grouped).map(
        ([ref, lignes]) => {
          const first = lignes[0];
          const demandePaiement = lignes
            .map((l) => {
          ...
          return {
            ...
            selectedDate: (first["Date de naissance Urssaf (AI)"] || "").split(
              "T"
            )[0],
      ...
      if (erreurs.length > 0) {
        const lignes = erreurs.map(
          (e) =>
            `- Facture : ${e.facture}, Code nature : ${
              e.nature
            }, Différence de ${e.diff.toFixed(2)}`
        );
        const message = [
      ...
      const formulairesImportés = Object.entries(grouped).map(
        ([ref, lignes]) => {
          ...
          return {
            ...
            selectedDate: first["dateNaissanceClient"]
              ? first["dateNaissanceClient"].split("T")[0]
              : today,
            dde: first["dateDebutEmploi"]
              ? first["dateDebutEmploi"].split("T")[0]
              : today,
            dfe: first["dateFinEmploi"]
              ? first["dateFinEmploi"].split("T")[0]
              : today,
            datevers: first["dateVersementAcompte"]
              ? first["dateVersementAcompte"].split("T")[0]
              : today,
            datefact: first["dateFacture"]
              ? first["dateFacture"].split("T")[0]
              : today,
            numfacture: ref,
  ...
  const genererJSON = () => {
    ...
    if (typeFichier === "dolibarr") {
      payload = formulaires.map((form) => ({
        //idTiersFacturation: identifiantAPI || "inconnu",
        ...
        inputPrestations: form.demandePaiement.map((p) => ({
          ...
          codeActivite:"",
          codeNature: String(p.cn), // Dolibarr → Libéllé Urssaf
          quantite: p.qte,
          unite: p.unit,
      ...
      payload = formulaires.map((form) => ({
        idClient: form.clientId,
        //idTiersFacturation: identifiantAPI || "inconnu",
        idTiersFacturation: "80077179200028",
        dateNaissanceClient: form.selectedDate + "T00:00:00Z",
        numFactureTiers: form.numfacture,
        dateFacture: form.datefact + "T00:00:00Z",
        dateDebutEmploi: form.dde + "T00:00:00Z",
    ...
    fetch("http://localhost:2083/demande/envoyer", {
      ...
      .then((data) => {
          ...
          if (Array.isArray(data)) {
            const doublons = data.filter(item => item.statut === "ERR_FACTURE_DOUBLON");

            if (doublons.length > 0) {
              const messages = doublons.map(item =>
                `La facture numéro ${item.numFactureTiers} est déjà existante`
              );
              alert(messages.join("\n"));
             } else {
                //  Nouveau message par facture envoyée
            ...
            // Cas où l'API renvoie un objet au lieu d'un tableau
            alert("Réponse inattendue de l'API URSSAF : " + JSON.stringify(data, null, 2));
          }
        });
          };
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                {resumeErreurs.map((line, i) => (
                  <p key={i} className={i < 2 ? "fw-bold mb-1" : "mb-1"}>
                    {line}
                  </p>
                ))}
              </div>
            ...
            <div className="mb-3">
              <div className="form-check form-check-inline">
                <input
                  ...
                  name="sourceType"
                  id="sourceFichier"
                  checked={typeFichier === "application"}
                  onChange={() => setTypeFichier("application")}
                />
                <label className="form-check-label" htmlFor="sourceFichier">
                  À partir d'un fichier de l'Application
                </label>
              </div>
              <div className="form-check form-check-inline">
                <input
                  ...
                  name="sourceType"
                  id="sourceApp"
                  checked={typeFichier === "dolibarr"}
                  onChange={() => setTypeFichier("dolibarr")}
                />
                <label className="form-check-label" htmlFor="sourceApp">
                  À partir d'une Exportation Dolibarr
                </label>
              </div>
            </div>
          </div>
        ...
        <div className="input-group mb-3">
          ...
          <div className="d-flex flex-wrap gap-2">
            <button
              ...
              onClick={() =>
                fichier && window.open(URL.createObjectURL(fichier))
              }
            >
              Ouvrir Excel
            </button>
          ...
          <div key={form.id} className="card mb-4 shadow-sm">
            ...
            <div className="card-body">
              <div className="row g-3 mb-3">
                ...
                <div className="col-md-6 col-12">
                  <label className="form-label">Date début emploi</label>
                  <input
                    type="date"
                    className="form-control"
                    value={form.dde}
                ...
                <div className="col-md-6 col-12">
                  ...
                  <input
                    type="number"
                    className="form-control"
                    value={form.mntacompte.toFixed(2)}
                    readOnly
                  />
                </div>
                <div className="col-md-6 col-12">
                ...
                <table className="table table-bordered table-sm">
                  ...
                  <tfoot>
                    <tr className="table-secondary fw-bold">
                      ...
                      <td></td>
                      <td>
                        {calculTotaux(form.demandePaiement).ht.toFixed(2)}
                      </td>
                      <td>
                        {calculTotaux(form.demandePaiement).ttc.toFixed(2)}
                      </td>
                      <td>
                        {calculTotaux(form.demandePaiement).tva.toFixed(2)}
                      </td>
                      <td colSpan="2"></td>
                    </tr>
                  </tfoot>
